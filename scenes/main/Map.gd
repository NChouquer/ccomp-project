...
var enemy_spawn_points = [Vector2(608,320),Vector2(96,800),Vector2(320,896)]
...
func SpawnEnemy():
	if enemy_list.size()>=enemy_maximum:
		...
		enemy_list[enemy_id_counter] = {"EnemyType": type, "EnemyLocation": location, "EnemyHealth":10,"EnemyMaxHealth":10,"EnemyState":"Idle","time_out":1}
...
func NPCHit(enemy_id,skill):
	if enemy_list[enemy_id]["EnemyHealth"]<=0:
		pass
	else:
		enemy_list[enemy_id]["EnemyHealth"] = enemy_list[enemy_id]["EnemyHealth"] - get_node("../Combat").getSkillDamage(skill)
		if enemy_list[enemy_id]["EnemyHealth"] <=0:
			print(enemy_list)
			print(enemy_id)
			...
			get_parent().get_node("WorldMap/Characters/Enemies/"+str(enemy_id)).queue_free()
